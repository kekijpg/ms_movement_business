...
public class MovementService {
    ...
    public List<Movement> findAll() {return repository.findAll();}
    //genera un Id modelo "M" + N++
    private String generateId() {
        Movement last = repository.findTopByOrderByIdDesc();
        if (last == null) {return "M001";}
        String lastId = last.getId().substring(1);
        int next = Integer.parseInt(lastId) + 1;
        return String.format("M%03d", next);}

    public Optional<Movement> findById(String id) {return repository.findById(id);}
    ...
    public Movement update(String id, Movement movement) {
        ...
        movement.setId(id);return repository.save(movement);}

    public void delete(String id) {repository.deleteById(id);}
    public List<Movement> findByCustomerId(String customerId) {return repository.findByCustomerId(customerId);}
    public List<Movement> findByProductId(String productId) {return repository.findByProductId(productId);}
}
